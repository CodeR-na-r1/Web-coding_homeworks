    ...
    for ($i=0; $i < count($_POST['selected_id']); $i++) {
      foreach ($content_rows as $key => $data) {
        if ( strcmp($_POST['selected_id'][$i], explode($separator, $data)[0]) == 0) {
          ...
          $edit_data = explode($separator, $data);
          $edit_data[10] = 0;
          $content_rows[$key] = implode($separator, $edit_data);
          break;
        }
      }
    }
      ...
      <table>
        ...
        <!-- Данные -->
          <?php foreach ($rows as $row): ?>
            <?php $data_row = explode($separator, trim($row)) ?>  <!-- Если нет нашего разделителя в данных, то функция вернет массив из одного элемента (ниже проверка) -->
